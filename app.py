...
# --- DỮ LIỆU THÍ SINH TINH HOA NHÍ (THEO CỔNG 1VOTE.VN) ---
VOTING_DATA_INIT = {
    "Ngôi Sao Nhí 2025": [
        {"name": "Nguyễn Vũ Minh Lam", "votes": 85620, "change": 0, "img": "https://via.placeholder.com/80?text=ML"},
        {"name": "Lý Thị Diệp Quỳnh (San San)", "votes": 72450, "change": 0, "img": "https://via.placeholder.com/80?text=SS"},
        {"name": "Hán Y Triệu Vy", "votes": 68900, "change": 0, "img": "https://via.placeholder.com/80?text=TV"},
        {"name": "Nguyễn Thái An Bình", "votes": 54200, "change": 0, "img": ""},
        {"name": "Bùi Đức Huy", "votes": 48300, "change": 0, "img": ""}
    ],
    "Nghệ Sĩ Nhí 2025": [
        {"name": "Quỳnh My", "votes": 45600, "change": 0, "img": "https://via.placeholder.com/80?text=QM"},
        {"name": "Hoàng Đài", "votes": 42300, "change": 0, "img": "https://via.placeholder.com/80?text=HD"},
        {"name": "Thùy Dương", "votes": 39800, "change": 0, "img": "https://via.placeholder.com/80?text=TD"},
        {"name": "Kim Ngân", "votes": 28900, "change": 0, "img": ""},
        {"name": "Bảo Anh", "votes": 12400, "change": 0, "img": ""}
    ]
}
...
def fetch_video_data_api(video_ids):
    ...
    try:
        ...
        for item in response['items']:
            ...
            try:
                dt = datetime.datetime.strptime(snippet['publishedAt'], "%Y-%m-%dT%H:%M:%SZ") + datetime.timedelta(hours=7)
...
# ==========================================
# --- 3. CSS TÙY CHỈNH (KHÔNG DÙNG F-STRING) ---
# ==========================================
...
<style>
    ...
    .podium-item { text-align: center; width: 110px; }
    .podium-rank { font-size: 24px; font-weight: 900; margin-bottom: 5px; }
    .podium-img { width: 70px; height: 70px; border-radius: 50%; border: 3px solid #FFF; object-fit: cover; }
    .podium-bar { border-radius: 10px 10px 0 0; display: flex; flex-direction: column; justify-content: center; color: #000; font-weight: bold; }
    ...
    .metric-val { font-size: 26px; font-weight: 800; color: white; }
...
if 'init_done' not in st.session_state:
    ...
    st.session_state['voting_sim'] = VOTING_DATA_INIT
...
with tab_vote:
    st.markdown('<div class="content-spacer"></div><div class="main-content">', unsafe_allow_html=True)
    vote_cat = st.selectbox("CHỌN HẠNG MỤC BÌNH CHỌN:", list(VOTING_DATA_INIT.keys()))
    vote_podium = st.empty()
    vote_table = st.empty()
    vote_chart = st.empty()
    st.markdown('</div>', unsafe_allow_html=True)
...
while True:
    ...
    if int(time.time()) % 60 == 0:
        df_new, lat_new = load_sheet_data()
        if lat_new is not None:
            st.session_state['latest'] = lat_new
            if int(lat_new['Youtube_View']) > st.session_state['total_view_sim']: st.session_state['total_view_sim'] = int(lat_new['Youtube_View'])
        st.session_state['video_data'] = fetch_video_data_api(VIDEO_IDS)
    ...
    if lat is not None:
        ...
        with video_placeholder.container():
            ...
            for i, vid_id in enumerate(VIDEO_IDS):
                if vid_id in st.session_state['video_data']:
                    d = st.session_state['video_data'][vid_id]
                    with cols[i]:
    ...
    # Render Voting
    curr_data = sorted(st.session_state['voting_sim'][vote_cat], key=lambda x: x['votes'], reverse=True)
    with vote_podium.container():
        if len(curr_data) >= 3:
            t1, t2, t3 = curr_data[0], curr_data[1], curr_data[2]
    ...
    with vote_table.container():
        rows = ""
        for i, item in enumerate(curr_data):
            badge = '<span class="badge-p">+' + str(item['change']) + '</span>' if item['change'] > 0 else '<span class="badge-n">0</span>'
            rows += '<div class="vote-row"><div class="col-rank">#' + str(i+1) + '</div><div class="col-name">' + item['name'] + '</div><div class="col-total">' + "{:,}".format(item['votes']) + '</div><div class="col-change">' + badge + '</div></div>'
        st.markdown('<div class="vote-table-card"><div class="vote-header-card">Best Fandom Forever</div><div class="vote-table-body">' + rows + '</div></div>', unsafe_allow_html=True)
