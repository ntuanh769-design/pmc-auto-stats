...
# Dữ liệu khởi tạo Voting (Giống hình bạn gửi)
INIT_VOTING_DATA = [
...
# CSS ĐƯỢC TÁCH RA KHỎI F-STRING ĐỂ TRÁNH LỖI CÚ PHÁP
custom_css = """
<style>
    ...
    /* FOOTER */
    .footer-container { 
        background: #4A148C; padding: 40px; margin-top: 60px; color: white; text-align: left; border-top: 1px solid #222;
    }
    ...
    .footer-bottom { 
        margin-top: 30px; border-top: 1px solid rgba(255,255,255,0.2); padding-top: 20px; 
        font-size: 13px; color: #E1BEE7; text-align: center; line-height: 1.6;
    }
...
# Placeholder cho Footer (Đặt ở ngoài cùng)
...
""", unsafe_allow_html=True)
    ...
    with vote_table_placeholder.container():
        ...
        for item in st.session_state['voting_data']:
            ...
            change_txt = f"+{item['change']}" if item['change'] > 0 else "0"
            rows_html += f"""
<div class="vote-row">
<div class="col-rank">#{item['rank']}</div>
<div class="col-name">{item['name']}</div>
<div class="col-total">{item['votes']:,}</div>
<div class="col-change"><span class="{change_cls}">{change_txt}</span></div>
</div>
"""
        st.markdown(f"""
...
""", unsafe_allow_html=True)
    ...
    # Render Footer (Trong vòng lặp để luôn hiện)
